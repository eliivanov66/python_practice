...
def logic():
    ...
    def startup(msg: telebot.types.Message):
        bot.send_message(chat_id=msg.from_user.id , text="Кнопка 'Start' бесполезна калькулятору")

    @bot.message_handler(commands=['stop'])
    def shutdown(msg: telebot.types.Message):
        bot.send_message(chat_id=msg.from_user.id , text="Кнопка 'Stop' калькулятор не остановить")

    @bot.message_handler(commands=['info'])
    def info(msg: telebot.types.Message):
        bot.send_message(chat_id=msg.from_user.id , text="Бот с калькулятором действительных чисел, введите ему формулу")
    ...
    def log(msg: telebot.types.Message):
        ...
        try:
            ...
            bot.send_message(chat_id=msg.from_user.id , text=f"Бот с калькулятором действительных чисел, введите ему формулу")
        else:
            for line in lines:
