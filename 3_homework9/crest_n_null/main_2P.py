...
def echo(msg: telebot.types.Message):
    ...
    if not game_running and msg.text.lower() != "играть":
        bot.send_message(chat_id=msg.from_user.id , text="Игра в крестики нолики, для начала игры напишите 'играть'") # показать текущее состояние поля
    if msg.text.lower() == "играть":
        ...
        if move:
            ...
            bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 1, выберите координату для '+' в формате x y" )  
            return
        else:
            bot.send_message(chat_id=msg.from_user.id , text=my_field_display(field_value)) # показать текущее состояние поля
            bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, выберите координату для 'o' в формате x y" )
            return 
    ...
    if move and game_running and not game_over: # ход игрока 1
        ...
        if player_1_choise == -1:
            bot.send_message(chat_id=msg.from_user.id , text="Ход игрока 1, Координаты должны быть в формате x y")
            bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 1, выберите координату для '+' в формате x y" )
            return
        else:
            if not my_check_empty(field_value, player_1_choise): # проверка заполнена ли выбранная координата
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 1, указанная координата {player_1_choise} занята")
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 1, выберите координату для '+' в формате x y" )
                return 
            if my_check_empty(field_value, player_1_choise):
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 1, игрок выбрал точку {player_1_choise}")
                my_set_field(field_value, player_1_choise, "+") # размещение на поле
                bot.send_message(chat_id=msg.from_user.id , text=my_field_display(field_value)) # показать текущее состояние поля
                move = 0 # передать ход человеку 2
                ...
                if game_over == 0:
                    bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, выберите координату для 'o' в формате x y" )
                    return 
    if not move and game_running and not game_over: # ход игрока 2 / компьютера
        player_2_choise = my_coord_get(msg.text) # выбор координаты игроком
        if player_2_choise == -1:
            bot.send_message(chat_id=msg.from_user.id , text="Ход игрока 2, координаты должны быть в формате x y")
            bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, выберите координату для 'o' в формате x y" )
            return
        else:
            if not my_check_empty(field_value, player_2_choise): # проверка заполнена ли выбранная координата
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, указанная координата {player_2_choise} занята")
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, выберите координату для 'o' в формате x y" )
                return 
            if my_check_empty(field_value, player_2_choise) and len(player_2_choise) == 2:
                bot.send_message(chat_id=msg.from_user.id , text=f"Ход игрока 2, игрок выбрал точку {player_2_choise}")
    ...
    if game_running:
        if game_over == 1:
            bot.send_message(chat_id=msg.from_user.id , text="Игрок 1 победил") # игрок 1
            game_running = 0
        elif game_over == 2:
            bot.send_message(chat_id=msg.from_user.id , text="Игрок 2 победил") # игрок 2
            game_running = 0
        else:
            bot.send_message(chat_id=msg.from_user.id , text="Партия закончилась ничьей") # игрок 2
            game_running = 0
    if not game_running and msg.text.lower() != "играть":
        bot.send_message(chat_id=msg.from_user.id , text="Игра в крестики нолики, для начала игры напишите 'играть'") # показать текущее состояние поля
    if msg.text.lower() == "играть":
